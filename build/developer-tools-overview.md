...
When building or integrating with these indexers, developers must account for unique blockDAG edge cases. Notably, Kaspa's DAA score is not strictly monotonic, which can create rare edge cases for state changes and ordering. Additionally, during UTXO state synchronization, clients must gracefully handle removed UTXOs that might not exist in the initial synced set. Finally, because achieving a perfectly clean state history from the network's earliest days is likely unsolvable due to a lack of early archival nodes, indexing services must be built to be resilient to potential gaps in very early historical data. To manage the high request volumes generated by these tools, developers are currently prioritizing the scaling of existing infrastructure (such as adding servers) over immediate architectural overhauls.
